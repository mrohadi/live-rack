...
/** Lerp two hex colours by t ∈ [0,1]. Used for heat fill. */
...
export function KonvaZoneCanvas({
  ...
  useEffect(() => {
    ...
    zones.forEach((zone) => {
      const fillPct =
        zone.items != null && zone.capacity ? Math.min(1, zone.items / zone.capacity) : 0.5;

      const fill =
        viewMode === "heat" ? lerpHex("#0f172a", zone.color, 0.15 + fillPct * 0.85) : zone.color;
      ...
      if (viewMode === "items") {
        ...
        const GAP = 2;
        const STEP = SLOT + GAP;
        const PAD = 24; // leave room for zone label at top

        const usableW = zone.width - 8;
        const usableH = zone.height - PAD - 4;
        const cols = Math.max(1, Math.floor(usableW / STEP));
        const rows = Math.max(1, Math.floor(usableH / STEP));
        const totalSlots = cols * rows;

        const filled = Math.min(totalSlots, Math.round(totalSlots * fillPct));
        ...
        for (let i = 0; i < totalSlots; i++) {
          ...
          layer.add(
            new Konva.Rect({
              ...
              width: SLOT,
              height: SLOT,
              fill: isFilled ? zone.color : "#1e293b",
      ...
      if ((viewMode === "heat" || viewMode === "items") && zone.items != null && zone.capacity) {
        layer.add(
          new Konva.Text({
            x: zone.x + 6,
            y: zone.y + 20,
            text: `${zone.items}/${zone.capacity} · ${Math.round(fillPct * 100)}%`,
            fontSize: 10,
            fontFamily: "monospace",
            fill: "#94a3b8",
            listening: false,
