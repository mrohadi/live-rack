...
describe("design tokens", () => {
  test("all required color tokens defined in light mode", () => {
    for (const token of REQUIRED_COLOR_TOKENS) {
      ...
      expect(lightTokens[token], `invalid HSL for light token: ${token}`).toMatch(HSL_RE);
    }
  });
  ...
  test("all required color tokens defined in dark mode", () => {
    for (const token of REQUIRED_COLOR_TOKENS) {
      ...
      expect(darkTokens[token], `invalid HSL for dark token: ${token}`).toMatch(HSL_RE);
    }
  });
  ...
  test("static tokens defined (font, radius)", () => {
    for (const token of REQUIRED_STATIC_TOKENS) {
      expect(staticTokens[token], `missing static token: ${token}`).toBeDefined();
